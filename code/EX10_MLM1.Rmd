...
- If the groups can be considered as a sample from some (real or hypothetical) population of groups, such as our county example, and you want to draw conclusions about this population of groups, then the random effects model is appropriate.
...
- BRFSS many years (pre 2014 county identifier, post 2014, MSA identifier)
...
head(data.frame(name=table(brfss_17$mmsaname),id=unique(brfss_17$mmsa)))
...
usacs<-usacs%>%
  ...
  dplyr::select(GEOID,NAME, totpop, pwhite, pblack, phisp, pfemhh, phsormore, punemp, medhhinc, ppov, pforn,plep, pvacant)


head(usacs)
```

I also have segregation measures at the county level that I made [here](http://rpubs.com/corey_sparks/116032), I will merge these in too.
...
```{r}
merged<-merge(x=brfss_17, y=merged, by.x="mmsa", by.y="GEOID", all.x=F)
...
merged$bmiz<-as.numeric(scale(merged$bmi, center=T, scale=T))
#merged<-merged[complete.cases(merged[, c("bmiz", "race_eth", "agec", "educ", "gini")]),]
#and merge the data back to the kids data

merged<-merged%>%
  dplyr::select(bmiz, obese, mmsa, agec, educ, race_eth,smoke, healthmdays, badhealth,bmi,pblack,phisp,pfemhh,phsormore, medhhinc,ppov, pforn,plep,pvacant, male, mmsawt, mmsaname )%>%
  filter(complete.cases(.))


head(merged[, c("bmiz", "male", "agec", "educ", "ppov","medhhinc", "pvacant", "mmsa")])
...
fit.an<-lm(bmiz~as.factor(mmsa), merged)
anova(fit.an)
```
...
```{r}
fit.ob<-glm(obese~as.factor(mmsa),family=binomial, merged)
...
fit<-glmer(bmiz~(1|mmsa), data=merged)
